...
요청: 사이트에 들어간 앱 화면을 Hi-Fi 페이지(2563:33747)의 `LOOP · IA Screen Flow` 안 최종 화면 캡처로 교체. 아이폰 목업은 `목업` 프레임에 있는 iPhone 17 인스턴스 사용.

반영:

- Hero 1장, S03 4장, S05 6장 = 11개 최종 화면 교체. 원본 매핑은 `docs/final-screen-sources.md`.
- 원본 iPhone 17 목업의 외곽 450×920·내부 402×874·오프셋 24/23 사용. Dynamic Island는 원본에 포함, 별도 노치 추가 없음.
- 긴 캡처는 늘리거나 압축하지 않고 실제 비율로 표시·내부 세로 스크롤 허용. 목업 레이어는 pointer-events none.
- Vite 콘텐츠 해시 에셋, 화면 코드·alt 최종본 명칭 반영. 기존 여정 구성·반응형 겹침 조정 유지.
- 1440/1920/390 캡처, 모든 화면·목업 로딩 정상 및 가로 넘침 없음. 빌드 통과. Figma 원본 디자인은 수정하지 않음.
